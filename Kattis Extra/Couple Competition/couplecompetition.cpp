...
void processAL(vector<vll>& AL, vll& nodes, stack<ll>& Stack, vll& max_num_idx, ll max_num, ll& i, bool check_max) {
    ...
    if (!Stack.empty()) {
        while (!Stack.empty() && nodes[Stack.top()] <= nodes[i]) { // pop until we get to the biggest node from the current nodes[i], if biggest reached then stop
            Stack.pop(); // by popping, we get to the biggest node in the stack from node[i]
        }
        if (!Stack.empty()) { // if stack empty that means there are no bigger numbers than node[i]
