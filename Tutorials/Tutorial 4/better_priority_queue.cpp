...
namespace better_priority_queue {
    ...
    protected:
        vector<size_t> id_to_heappos;
        vector<priority_queue_node<Key, Priority>> heap;
    ...
    public:
        ...
        /** first is priority, second is key */
        const priority_queue_node<Key, Priority>& bottom() const { return heap.back(); }

        void pop(bool remember_key = false) {
            if (size() == 0) return;
            id_to_heappos[heap.front().key] = -1 - remember_key;
            if (size() > 1) {
                *heap.begin() = move(*(heap.end() - 1));
        ...
        priority_queue_node<Key, Priority> pop_value(bool remember_key = true) {
            ...
            priority_queue_node<Key, Priority> ret = move(*heap.begin());
        ...
        pair<bool, Priority> get_priority(const Key& key) {
        ...
        /** Returns true if the key was already inside and was successfully decreased my decrement amt, otherwise does nothing and returns false */
        bool decrease_priority(const Key& key, const Priority& decrement, bool only_if_higher = false) {
            if (key >= id_to_heappos.size()) return false;
            size_t heappos = id_to_heappos[key];
            if (heappos >= ((size_t)-2)) return false;
            Priority& priority = heap[heappos].priority;
            Priority amt = heap[heappos].priority;
            Priority new_priority = amt - decrement;
            if (new_priority > priority) {
                priority = new_priority;
                sift_up(heappos);
                return true;
    ...
    private:
        ...
        void sift_down(size_t heappos) {
            ...
            priority_queue_node<Key, Priority> val = move(heap[heappos]);
            do {
                heap[heappos] = move(heap[child]);
...
int main()
{
    better_priority_queue::updatable_priority_queue<ll, ll> pQ;
    // <node, weight> where each node is a unique number
    pQ.push(0, 30);   // or pQ.set(0, 30)
    pQ.push(1, 20);
    pQ.push(2, 10);
    pQ.push(2, 50); // duplicates are ignored
    pQ.push(3, 100);

    pQ.update(2, 25); // or pQ.set(2, 25)
    pQ.decrease_priority(3, 99); // 100 - 99 = 1 priority

    // !NOTE: by default its a MAX HEAP
    // !if you want MIN HEAP, just take bottom instead of top()
    // !in fact, the PQ is very much like a gnu_pbds but more limited in features and probably slower
    cout << "top of Max heap is: <" << pQ.top().key << ", " << pQ.top().priority << ">" << endl; // <0, 30>
    cout << "bottom of Max heap is: <" << pQ.bottom().key << ", " << pQ.bottom().priority << ">" << endl; // <3, 1>

    while (!pQ.empty())
        cout << pQ.pop_value().key << ' '; // 0 2 1 3
